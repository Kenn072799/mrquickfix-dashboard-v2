...
const getFilteredProjects = (
  ...
  const filtered = projects.filter((project) => {
    ...
    const isInDateRange =
      (!adjustedStartDate || cancelledDate >= adjustedStartDate) &&
      (!adjustedEndDate || cancelledDate <= adjustedEndDate);
  ...
  return [...filtered].sort((a, b) => {
    ...
    const aValue =
      sortConfig.key === "name"
        ...
        : sortConfig.key === "date"
          ? new Date(a.jobCancelledDate)
          : a[sortConfig.key];
    const bValue =
      sortConfig.key === "name"
        ? `${b.clientFirstName} ${b.clientLastName}`
        : sortConfig.key === "date"
          ? new Date(b.jobCancelledDate)
          : b[sortConfig.key];
...
const CancelledTableReport = ({ projects, loading, error }) => {
  ...
  const handleCSVDownload = () => {
    const csvData = paginatedProjects.map((project) => ({
      ...
      Date: formatDate(project.jobCancelledDate),
    }));
  ...
  const handlePDFDownload = () => {
    ...
    const rows = filteredProjects.map((project, index) => [
      ...
      formatDate(project.jobCancelledDate),
    ]);
    ...
    const dateRange =
      formattedStartDate || formattedEndDate
        ? `From ${formattedStartDate} - ${formattedEndDate}`
        : "All Dates";
        ...
        <table className="min-w-full divide-y-2 divide-secondary-100 border-b border-secondary-100 text-sm">
          <thead className="text-left">
            <tr className="border-b-2 border-secondary-200 text-sm text-primary-500">
              ...
              <th
                className={`group relative cursor-pointer px-4 py-2 hover:bg-secondary-50 ${
                  sortConfig.key === "jobCancellationReason"
                    ? "bg-secondary-50"
                    : ""
                }`}
                onClick={() => requestSort("jobCancellationReason")}
              >
                <div className="flex items-center justify-between gap-2">
                  <span>Reason</span>
                  {getSortDirection("jobCancellationReason")}
                </div>
              </th>
              <th
